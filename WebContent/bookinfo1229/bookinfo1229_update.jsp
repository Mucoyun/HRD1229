...
<head>
	...
	<script>
		function Click() {
			if(document.bookinfo_U_form.bookno.value==""){
				alert("도서코드가 입력되지 않았습니다.");
				document.bookinfo_U_form.bookno.focus();
			}else if(document.bookinfo_U_form.author.value==""){
				alert("저자가 입력되지 않았습니다.");
				document.bookinfo_U_form.author.focus();
			}else if(document.bookinfo_U_form.bookname.value==""){
				alert("도서이름이 입력되지 않았습니다.");
				document.bookinfo_U_form.bookname.focus();
			}else{
				alert("수정이 완료되었습니다.");
				document.bookinfo_U_form.submit();
			}
		}
	</script>
</head>
<body>
	...
	<section>
		<h2>도서 정보 등록 화면</h2>
		<form name="bookinfo_U_form" method="post" action="bookinfo1229_update_process.jsp">
			<table id="bookinfo_IU_Table">
				...
				<tr>
					<td class="btn_td" colspan="2">
						...
						<button type="button" onclick="location.href='/HRD_1229/bookinfo1229/bookinfo1229_select.jsp'">뒤로</button>
					</td>
				</tr>
			</table>
		</form>
